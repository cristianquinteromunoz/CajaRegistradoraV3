...
class FacturacionFrame(ctk.CTkFrame):
    def __init__(self, parent, fuentes, estilos, colores, id_usuario=1, **kwargs):
        ...
        self.items_factura = []   # lista de dicts con los items actuales
    ...
    def _construir_panel_izquierdo(self):
        ...
        # Título
        ctk.CTkLabel(panel, text="Nueva Factura",
        ...
        self.entrada_busqueda = ctk.CTkEntry(
            busq, placeholder_text="Nombre o código de barras...",
            **self.E["entry"]
        )
        ...
        self.lista_resultados.grid(
            row=2, column=0, columnspan=3, sticky="ew", padx=14, pady=(0, 14)
        )
    ...
    def _construir_tabla_items(self, parent):
        ...
        contenedor.grid(row=2, column=0, sticky="nsew", pady=(0, 0))
        ...
        style.configure("Fact.Treeview",
            ...
            rowheight=32, font=("Roboto", 12), borderwidth=0,
        )
        ...
        self.tabla_items = ttk.Treeview(
            ...
            style="Fact.Treeview", selectmode="browse"
        )
        config = {
            "tipo":        ("Tipo",         70,  "center"),
            "nombre":      ("Nombre",       200, "w"),
    ...
    def _construir_panel_derecho(self):
        ...
        def fila_total(fila, label, var, color=None):
        ...
        fila_total(0, "Subtotal:",   self.var_subtotal)
        fila_total(1, "Descuento:",  self.var_descuento, self.C["exito"])
        ttk.Separator(totales, orient="horizontal").grid(
            row=2, column=0, columnspan=2, sticky="ew", padx=10, pady=4
        )
        fila_total(3, "TOTAL:",      self.var_total, self.C["acento"])
        ...
        for i, (texto, valor) in enumerate(metodos):
            ctk.CTkRadioButton(
                pago_frame, text=texto, value=valor,
                variable=self.var_metodo,
                command=self._toggle_mixto,
                font=self.F["cuerpo_md"],
                text_color=self.C["texto_primario"],
                fg_color=self.C["acento"],
            ).grid(row=i // 2, column=i % 2, sticky="w", pady=4)
        ...
        self.frame_mixto.grid_remove()  # oculto por defecto

        for i, (label, attr) in enumerate([("Efectivo $", "e_mixto_efectivo"),
                                            ("Tarjeta $",  "e_mixto_tarjeta"),
                                            ("Transfer $", "e_mixto_transfer")]):
            ctk.CTkLabel(self.frame_mixto, text=label, **self.E["label_cuerpo"]).grid(
                row=i, column=0, sticky="w", pady=2
            )
            entry = ctk.CTkEntry(self.frame_mixto, **self.E["entry"])
        ...
        ctk.CTkButton(
            ...
            height=50,
            font=self.F["titulo_sm"],
            fg_color=self.C["exito"],
            hover_color="#388e3c",
            text_color="#ffffff",
            corner_radius=10,
    ...
    def _buscar(self, *_):
        ...
        if self.var_tipo.get() == "Producto":
            resultados = self.prod_svc.buscar(texto)
    ...
    def _fila_resultado_producto(self, producto):
        ...
        ctk.CTkLabel(
            frame,
            text=f"[{producto.codigo_barras or '—'}]",
        ...
        ctk.CTkLabel(
            frame,
            text=f"Stock: {producto.stock_actual}",
            font=self.F["cuerpo_sm"],
            text_color=self.C["texto_secundario"],
    ...
    def _fila_resultado_servicio(self, servicio):
        ...
        ctk.CTkButton(
            ...
            hover_color=self.C["acento_hover"],
            font=self.F["boton"],
            command=lambda s=servicio: self._agregar_servicio(s),
        ).grid(row=0, column=2, padx=8, pady=4)

    # ==========================================================
    #  SELECCIÓN DE PRODUCTO — elegir presentación y descuento
    # ==========================================================
    def _seleccionar_producto(self, producto):
        ...
        if not presentaciones:
            messagebox.showwarning("Sin presentaciones",
                f"'{producto.nombre}' no tiene presentaciones configuradas.\n"
                "Agrégalas desde el módulo de Inventario.", parent=self)
            return
        ...
        combo_pres = ctk.CTkComboBox(d, values=nombres_pres, variable=var_pres,
                                     font=self.F["cuerpo_md"])
        ...
        def actualizar_label_dto(*_):
            ...
            if p.precio_con_descuento:
                lbl_dto.configure(
                    text=f"Aplicar descuento ({p.porcentaje_descuento:.1f}% → ${p.precio_con_descuento:,.0f})"
                )
            else:
                lbl_dto.configure(text="Sin descuento configurado")
                var_dto.set(False)

        lbl_dto = ctk.CTkLabel(d, text="", font=self.F["cuerpo_sm"],
                               text_color=self.C["texto_secundario"])
        chk_dto = ctk.CTkCheckBox(d, textvariable=ctk.StringVar(),
                                  variable=var_dto,
                                  fg_color=self.C["exito"],
                                  font=self.F["cuerpo_md"],
                                  text_color=self.C["texto_primario"])
    ...
    def _refrescar_tabla(self):
        ...
        for item in self.items_factura:
            dto_txt = (f"-{item['porcentaje_descuento']:.1f}%"
                       if item["descuento_aplicado"] else "—")
    ...
    def _quitar_item(self):
        ...
        if not sel:
            messagebox.showwarning("Atención", "Selecciona un item para quitar.", parent=self)
            return
        idx = self.tabla_items.index(sel[0])
    ...
    def _cobrar(self):
        if not self.items_factura:
            messagebox.showwarning("Factura vacía",
                "Agrega al menos un producto o servicio.", parent=self)
            return
        ...
        venta = self.ventas_svc.crear_venta(
            ...
            metodo_pago = metodo if metodo != "mixto" else "mixto",
            items       = [
                {
        ...
        if venta:
            ...
            messagebox.showerror("Error",
                "No se pudo registrar la venta. Verifica el stock disponible.", parent=self)